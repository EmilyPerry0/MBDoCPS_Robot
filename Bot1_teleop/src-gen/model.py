...
class Model:
	...
	def is_state_active(self, state):
		...
		"""
		s = state
		if s == self.__State.cwait_for_command:
			return self.__state_vector[0] == self.__State.cwait_for_command
		if s == self.__State.cmove_forward:
			return self.__state_vector[0] == self.__State.cmove_forward
		if s == self.__State.cstop:
			return self.__state_vector[0] == self.__State.cstop
		if s == self.__State.cmove_backward:
			return self.__state_vector[0] == self.__State.cmove_backward
		if s == self.__State.crotate_right:
			return self.__state_vector[0] == self.__State.crotate_right
		if s == self.__State.crotate_left:
			return self.__state_vector[0] == self.__State.crotate_left
		return False
	...
	def __entry_action_c_move_forward(self):
		""".
		"""
	...
	def __entry_action_c_stop(self):
		""".
		"""
	...
	def __entry_action_c_move_backward(self):
		""".
		"""
		#Entry action for state 'Move_Backward'.
		self.output.speed = (self.user_var.base_speed * -(2))
		self.__completed = True
		
	def __entry_action_c_rotate_right(self):
		""".
		"""
		#Entry action for state 'Rotate_Right'.
		self.output.rotation = self.user_var.base_rotation
		self.__completed = True
		
	def __entry_action_c_rotate_left(self):
		""".
		"""
		#Entry action for state 'Rotate_Left'.
		self.output.rotation = (self.user_var.base_rotation * -(1))
		self.__completed = True
		
	def __enter_sequence_c_wait_for_command_default(self):
		"""'default' enter sequence for state Wait_For_Command.
		"""
		#'default' enter sequence for state Wait_For_Command
		self.__state_vector[0] = self.State.cwait_for_command
		self.__state_conf_vector_changed = True
		
	def __enter_sequence_c_move_forward_default(self):
		"""'default' enter sequence for state Move_Forward.
		"""
		#'default' enter sequence for state Move_Forward
		self.__entry_action_c_move_forward()
		self.__state_vector[0] = self.State.cmove_forward
		self.__state_conf_vector_changed = True
		
	def __enter_sequence_c_stop_default(self):
		"""'default' enter sequence for state Stop.
		"""
		#'default' enter sequence for state Stop
		self.__entry_action_c_stop()
		self.__state_vector[0] = self.State.cstop
		self.__state_conf_vector_changed = True
		
	def __enter_sequence_c_move_backward_default(self):
		"""'default' enter sequence for state Move_Backward.
		"""
		#'default' enter sequence for state Move_Backward
		self.__entry_action_c_move_backward()
		self.__state_vector[0] = self.State.cmove_backward
		self.__state_conf_vector_changed = True
		
	def __enter_sequence_c_rotate_right_default(self):
		"""'default' enter sequence for state Rotate_Right.
		"""
		#'default' enter sequence for state Rotate_Right
		self.__entry_action_c_rotate_right()
		self.__state_vector[0] = self.State.crotate_right
		self.__state_conf_vector_changed = True
		
	def __enter_sequence_c_rotate_left_default(self):
		"""'default' enter sequence for state Rotate_Left.
		"""
		#'default' enter sequence for state Rotate_Left
		self.__entry_action_c_rotate_left()
		self.__state_vector[0] = self.State.crotate_left
		self.__state_conf_vector_changed = True
	...
	def __exit_sequence_c_wait_for_command(self):
		"""Default exit sequence for state Wait_For_Command.
		"""
		#Default exit sequence for state Wait_For_Command
		self.__state_vector[0] = self.State.null_state
		
	def __exit_sequence_c_move_forward(self):
		"""Default exit sequence for state Move_Forward.
		"""
		#Default exit sequence for state Move_Forward
		self.__state_vector[0] = self.State.null_state
		
	def __exit_sequence_c_stop(self):
		"""Default exit sequence for state Stop.
		"""
		#Default exit sequence for state Stop
		self.__state_vector[0] = self.State.null_state
		
	def __exit_sequence_c_move_backward(self):
		"""Default exit sequence for state Move_Backward.
		"""
		#Default exit sequence for state Move_Backward
		self.__state_vector[0] = self.State.null_state
		
	def __exit_sequence_c_rotate_right(self):
		"""Default exit sequence for state Rotate_Right.
		"""
		#Default exit sequence for state Rotate_Right
		self.__state_vector[0] = self.State.null_state
		
	def __exit_sequence_c_rotate_left(self):
		"""Default exit sequence for state Rotate_Left.
		"""
		#Default exit sequence for state Rotate_Left
		self.__state_vector[0] = self.State.null_state
	...
	def __exit_sequence_c(self):
		...
		if state == self.State.cwait_for_command:
			self.__exit_sequence_c_wait_for_command()
		elif state == self.State.cmove_forward:
			self.__exit_sequence_c_move_forward()
		elif state == self.State.cstop:
			self.__exit_sequence_c_stop()
		elif state == self.State.cmove_backward:
			self.__exit_sequence_c_move_backward()
		elif state == self.State.crotate_right:
			self.__exit_sequence_c_rotate_right()
		elif state == self.State.crotate_left:
			self.__exit_sequence_c_rotate_left()
		
	def __react_c__entry_default(self):
		"""Default react sequence for initial entry .
		"""
		#Default react sequence for initial entry 
		self.__enter_sequence_c_wait_for_command_default()
		
	def __c_wait_for_command_react(self, transitioned_before):
		"""Implementation of __c_wait_for_command_react function.
		"""
		...
		if not self.__do_completion:
			if transitioned_after < 0:
				if self.computer.w_press:
					self.__exit_sequence_c_wait_for_command()
					self.__enter_sequence_c_move_forward_default()
					transitioned_after = 0
				elif self.computer.s_press:
					self.__exit_sequence_c_wait_for_command()
					self.__enter_sequence_c_stop_default()
					transitioned_after = 0
				elif self.computer.x_press:
					self.__exit_sequence_c_wait_for_command()
					self.__enter_sequence_c_move_backward_default()
					transitioned_after = 0
				elif self.computer.d_press:
					self.__exit_sequence_c_wait_for_command()
					self.__enter_sequence_c_rotate_right_default()
					transitioned_after = 0
				elif self.computer.a_press:
					self.__exit_sequence_c_wait_for_command()
					self.__enter_sequence_c_rotate_left_default()
					transitioned_after = 0
	...
	def __c_move_forward_react(self, transitioned_before):
		"""Implementation of __c_move_forward_react function.
		"""
	...
	def __c_stop_react(self, transitioned_before):
		"""Implementation of __c_stop_react function.
		"""
	...
	def __c_move_backward_react(self, transitioned_before):
		"""Implementation of __c_move_backward_react function.
		"""
		#The reactions of state Move_Backward.
		transitioned_after = transitioned_before
		if self.__do_completion:
			#Default exit sequence for state Move_Backward
			self.__state_vector[0] = self.State.null_state
	...
	def __c_rotate_right_react(self, transitioned_before):
		"""Implementation of __c_rotate_right_react function.
		"""
		#The reactions of state Rotate_Right.
		transitioned_after = transitioned_before
		if self.__do_completion:
			#Default exit sequence for state Rotate_Right
			self.__state_vector[0] = self.State.null_state
	...
	def __c_rotate_left_react(self, transitioned_before):
		"""Implementation of __c_rotate_left_react function.
		"""
		#The reactions of state Rotate_Left.
		transitioned_after = transitioned_before
		if self.__do_completion:
			#Default exit sequence for state Rotate_Left
			self.__state_vector[0] = self.State.null_state
	...
	def __micro_step(self):
		...
		state = self.__state_vector[0]
		if state == self.State.cwait_for_command:
			self.__c_wait_for_command_react(-1)
		elif state == self.State.cmove_forward:
			self.__c_move_forward_react(-1)
		elif state == self.State.cstop:
			self.__c_stop_react(-1)
		elif state == self.State.cmove_backward:
			self.__c_move_backward_react(-1)
		elif state == self.State.crotate_right:
			self.__c_rotate_right_react(-1)
		elif state == self.State.crotate_left:
			self.__c_rotate_left_react(-1)
